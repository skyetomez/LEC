...
$$V^*_{\pi^*}(x^{(\rho-1)}) = \alpha^{(\rho-1)}(x^{(\rho-1)}, \pi^*(x^{(\rho-1)})) + \mathbb{E} \left[ V^*_{\pi^*}(x^{(\rho)}  x^{(\rho-1)}, \pi^*(x^{(\rho-1)})) \right]$$

Equation then allows us to extract the optimal policy $\pi^*$ at each state by:

$$
\pi^*(x^{(\rho-1)}) =  \left\{ \alpha^{(\rho-1)} + \mathbb{E} \left[ V^*_{\pi^*}(x^{(\rho)}  x^{(\rho-1)}, u^{(\rho-1)}) \right] \right\}
$$
